...
\end{align}
where $\hat{x}_{s}$ is the output of the NN for state $s$, $w_{m}^{\text {ode }}$ are the ODE weights and $f_s$ is the solution to the ODE system with the given parameters $\mathbf{p}$.
