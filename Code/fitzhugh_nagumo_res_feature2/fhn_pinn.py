...
def create_data(data_t, data_y):
    ...
    tau = tf.math.softplus(tf.Variable(0, trainable=True, dtype=tf.float32)) * 15
    Iext = tf.math.softplus(tf.Variable(0, trainable=True, dtype=tf.float32)) * 0.15
...
def main():
    ...
    noise = 0.0
    # tf.device("gpu")
    savename = Path("fitzhugh_nagumo_res_feature2")
