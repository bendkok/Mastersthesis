...
def main():
    ...
    for varr in range(len(varses0)):
        ...
        for i in range(10):
            cur_res = run_pinn(states=[0], var=varses0[varr], epochs=4e5, noise=noises[0], expe_n=i)
    cur_res = run_pinn(states=[0], var=[True,True,True,True], epochs=4e5, noise=.0)

    # noises     = [.00,.01,.02,.05,.10]
    # varses0  = [[True,False,False,False], [True,True,True,True]] #[True,True,False,False], 
    # for varr in range(len(varses0)):
    #     for nos in range(len(noises)):
    #         for i in range(10):
    #             cur_res = run_pinn(states=[0,1], var=varses0[varr], epochs=4e5, noise=noises[nos], expe_n=i)
