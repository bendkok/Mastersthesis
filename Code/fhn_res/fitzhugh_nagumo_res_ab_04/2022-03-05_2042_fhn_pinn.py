...
def create_observations(data_t, data_y, geom, savename):
    ...
    np.savetxt(
        os.path.join(savename, "fitzhugh_nagumo_pred.dat"), np.hstack((idx, data_t[idx].ravel(), data_y[idx,0], data_y[idx,1]))
    )
...
def main():
    ...
    start = time.time()
    noise = 0.1
    savename = Path("fhn_res/fitzhugh_nagumo_res_ab_04")
    ...
    var_list = pinn(
        ...
        sec_num_epochs=int(9e5),
        var_trainable=[True, True, False, False], #a, b, tau, Iext 
