...
def create_data(data_t, data_y, savename, var_trainable=[True, True, False, False], var_modifier=[-.25, 1.1, 20, 0.23], 
    ...
    observe_y0 = create_observations(data_t, data_y, geom, savename, observed_states=observed_states)
    
    data = dde.data.PDE(  
        geom,
        ODE,
        [bc0, bc1, observe_y0],  # list of boundary conditions
...
def pinn(
    ...
    create_hyperparam_dict(savename, first_num_epochs, sec_num_epochs, var_trainable, 
                           var_modifier, lr, decay_amount, init_weights, k_vals, #should propably have sent in weights here
...
def run_pinn(states=[0,1], var=[True,True,False,False], epochs=int(2e5), noise=0.01):
    ...
    var_list = pinn(
        ...
        init_weights = [[20., 20.], [10., 10.], d_w], # [[ode], [bc], [data]]
...
def main():
    ...
    for varr in range(len(varses)):
        for nos in range(len(noises)):
            run_pinn(states=[0], var=varses[varr], epochs=eps[nos], noise=noises[nos])
