...
def create_observations(data_t, data_y, geom, savename, observalbe_states=[0,1]):
    ...
    observalbe_states=np.array(observalbe_states)
    observalbe_states=np.array([0])
    # tmp = np.vstack((data_y[idx,0], data_y[idx,1]))
    samp_y = data_y[idx][:,observalbe_states]
    print(samp_y.shape, samp_y.ravel().shape)
    # for i in range(1,len(observalbe_states)):
    #     np.append(tmp, data_y[idx,observalbe_states[i]])
