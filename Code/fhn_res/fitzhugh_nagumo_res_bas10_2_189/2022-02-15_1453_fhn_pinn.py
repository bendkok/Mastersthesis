...
def train_model(model, weights, callbacks, first_num_epochs = int(1e3), 
    ...
    model.compile(
        ...
        decay=("inverse time", int(sec_num_epochs), 1e4),
    )
...
def main():
    ...
    start = time.time()
    noise = 0.0
    savename = Path("fhn_res/fitzhugh_nagumo_res_bas10_2_189")
    ...
    var_list = pinn(
        ...
        restore=False,
        first_num_epochs=0,
        sec_num_epochs=int(2e4),
        var_trainable=[False, True, False, False], #a, b, tau, Iext 
        var_modifier=[-.3, .5, 20, 0.23], #a, b, tau, Iext
        # init_weights = [[0, 0], [0, 0], [1, 1]], # [[ode], [bc], [data]]
        init_weights = [[.2, 10], [1e-1, 1e0], [.04, .1]], # [[ode], [bc], [data]]
...
def plot_features():
    ...
    fig, ax = plt.subplots()
    ax.plot(t, y[:,0])
    # for i in np.linspace(.2, 1.6, 12):
    #     y = fitzhugh_nagumo_model(t, b=i)
