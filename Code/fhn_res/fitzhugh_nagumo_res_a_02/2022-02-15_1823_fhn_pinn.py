...
def train_model(model, weights, callbacks, first_num_epochs = int(1e3), 
    ...
    model.compile(
        ...
        decay=("inverse time", int(sec_num_epochs), 1e2),
    )
...
def main():
    ...
    start = time.time()
    noise = 0.0
    savename = Path("fhn_res/fitzhugh_nagumo_res_a_02")
    ...
    var_list = pinn(
        ...
        var_modifier=[-.12, 1.1, 20, 0.23], #a, b, tau, Iext
...
def plot_features():
    ...
    y = fitzhugh_nagumo_model(t, a = -.12)
    fig, ax = plt.subplots()
    ax.plot(t, y[:,0])
