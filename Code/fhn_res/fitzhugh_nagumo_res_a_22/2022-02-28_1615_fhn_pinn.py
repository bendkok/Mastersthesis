...
def create_observations(data_t, data_y, geom, savename):
    ...
    print(idx.shape, data_t[idx].shape, data_y[idx].shape, data_y[idx,0].shape)
    
    np.savetxt(
        os.path.join(savename, "fitzhugh_nagumo_pred.dat"), np.hstack((idx, data_t[idx], data_y[idx,0], data_y[idx,1]))
    )
...
def main():
    ...
    start = time.time()
    noise = 0.0
    savename = Path("fhn_res/fitzhugh_nagumo_res_a_22")
