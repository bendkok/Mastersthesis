...
def create_data(data_t, data_y):
    ...
    a = tf.math.softplus(tf.Variable(0, trainable=True, dtype=tf.float32)) * - 0.2
    b = tf.math.softplus(tf.Variable(0, trainable=True, dtype=tf.float32)) * 1.2
    tau = tf.math.softplus(tf.Variable(0, trainable=True, dtype=tf.float32)) * 18
    Iext = tf.math.softplus(tf.Variable(0, trainable=True, dtype=tf.float32)) * 0.2
