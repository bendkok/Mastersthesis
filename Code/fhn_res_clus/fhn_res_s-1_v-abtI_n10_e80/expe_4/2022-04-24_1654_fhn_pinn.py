...
def pinn(
    ...
    data_t0 = np.linspace(1000,1999,1000)[:, None]
    nn_pred0 = model.predict( data_t0 ) 
    np.savetxt(
        os.path.join(savename, "neural_net_pred_out_dom.dat"), np.hstack((data_t0, nn_pred0))
    )
...
def main():
    ...
    # # varses0  = [[False,True,True,False]] #
    # # for varr in range(len(varses0)):
    #     # for nos in range(len(noises)):
    # for i in range(10):
    #     cur_res = run_pinn(states=[0], var=varses0[2], epochs=4e5, noise=noises[4], expe_n=i)
    # # cur_res = run_pinn(states=[0], var=[True,True,True,True], epochs=4e5, noise=.0)

    noises   = [.00,.01,.02,.05,.10]
    varses0  = [[True,False,False,False], [True,True,False,False], [True,True,True,True]] #
    # for varr in range(len(varses0)):
    # for nos in range(len(noises)):
    # for i in range(5,10):
    cur_res = run_pinn(states=[1], var=varses0[2], epochs=8e5, noise=noises[int(sys.argv[1])], expe_n=int(sys.argv[2]))

    # cur_res = run_pinn(states=[0], var=varses0[2], epochs=10e5, noise=noises[0], expe_n=0)
    # cur_res = run_pinn(states=[0,1], var=varses0[2], epochs=4e5, noise=noises[4], expe_n=4)
