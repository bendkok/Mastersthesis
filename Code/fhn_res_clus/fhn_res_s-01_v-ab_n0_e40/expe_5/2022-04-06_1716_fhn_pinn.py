...
def run_pinn(states=[0,1], var=[True,True,False,False], epochs=int(2e5), noise=0.01, expe_n=0):
    ...
    savename = Path("fhn_res/fhn_res_s-{}_v-{}_n{}_e{}".format(st, va, no, ep, expe_n))
...
def main():
    ...
    # for varr in range(len(varses0)):
        # for nos in range(len(noises)):
    # for i in range(5,10):
    #     cur_res = run_pinn(states=[0,1], var=varses0[1], epochs=4e5, noise=noises[0], expe_n=i)

    cur_res = run_pinn(states=[0,1], var=varses0[1], epochs=4e5, noise=noises[0], expe_n=5)
