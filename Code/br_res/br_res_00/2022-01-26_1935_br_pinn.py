...
def train_model(
    ...
    model.train(
        ...
        display_every=int(first_num_epochs),
...
def pinn(
    ...
    # It also returns an error because the model folder isn't created.
...
def main():
    ...
    var_list = pinn(
        ...
        restore=False,
        first_num_epochs=0,
        sec_num_epochs=int(100),
        # E_Na, g_Na, g_Nac, g_s, IstimAmplitude, IstimEnd, IstimPeriod,
        # IstimPulseDuration, IstimStart, C
        var_trainable=[False, True, True, True, False, False, False, False, False, False],
        var_modifier=[1]*10,#[50.0, 0.01, 1e-05, 0.0001, 0.5, 50000.0, 1000.0, 1.0, 10.0, 0.01],
        #m, h, j, Cai, d, f, x1, V
        ode_weights=[0]*8,#[1, 1, 1, 0, 1, 1, 1, 0],
        bc_weights=[0]*8,#[1, 1, 1, 1, 1, 1, 1, 1],
        data_weights=[0]*8,#[1, 1, 1, 1, 1, 1, 1, 1],
        k_vals=[1e-3], # tf.sin(k * 2*np.pi*t),
        do_output_transform=False,
        do_t_input_transform=True,
    ...
    plot_losses(savename, states = "m, h, j, Cai, d, f, x1, V".split(", "))
