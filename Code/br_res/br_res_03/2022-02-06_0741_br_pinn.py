...
def main():
    ...
    start = time.time()
    noise = 0.0
    savename = Path("br_res/br_res_03")
    ...
    var_list = pinn(
        ...
        var_modifier=[0.01, 1e-05, 0.0001],
        ode_weights=[1e-2, 1, 1, 1e-2, .1, .1, 1, 1e-6],
        bc_weights=[1e-2, 1e-2, 1e-2, 1e-2, 1e-4, 1e-4, 10, 10],
        data_weights=[.1, .1, .1, .1, 1e-3, 1e-3, .1, .01], #[1, 1, 1, 1, 1, 1, 1, 1],
        ...
        nn_layers=3,
        nn_nodes=128,
        display_every=int(1e0),
    )

    print(type(true_values))
    params = np.concatenate(np.concatenate([true_values[0]], var_list), true_values[4:]) #[0.5, 50000.0,1000.0,2.0,10.0, 0.01]
