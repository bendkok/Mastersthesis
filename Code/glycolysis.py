...
def main():
    ...
    savename = "glycolysis_res"

    # Data
    y = glycolysis_model(np.ravel(t))
    np.savetxt(os.path.join(savename, "glycolysis.dat"), np.hstack((t, y)))
    # Add noise
    if noise > 0:
        std = noise * y.std(0)
        y[1:-1, :] += np.random.normal(0, std, (y.shape[0] - 2, y.shape[1]))
        np.savetxt(os.path.join(savename, "glycolysis_noise.dat"), np.hstack((t, y)))
    ...
    np.savetxt(os.path.join(savename, "glycolysis_pred.dat"), np.hstack((t, y)))
