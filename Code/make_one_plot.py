...
def get_hyperparam_title(path):
    ...
    #weights = "Weights = [{}, {}, {}]".format(hyp[0][13:], hyp[1][12:], hyp[2][14:])
...
def plot_losses(path):
    ...
    #parts = ['BC', 'Data', 'ODE']
    parts = ['ODE', 'BC', 'Data']
    #I think bc is actually the second loss part, but the first weight part
    #Data is number three, which means ode is first
    ...
    for i in range(len(parts)):
        ...
        mean = np.mean((train_loss[:,i*2:i*2+2]), axis=1)#, axis=1)#, test_loss[:,i*2:i*2+2]), axis=0), axis=1)
    ...
    #     plt.plot(epochs, train_loss[:,i], "--", label="{}".format(i))
    # plt.plot(epochs, np.mean(train_loss, axis=1), label="Mean")
    # plt.yscale("log")
    # plt.legend()
    # plt.show()
